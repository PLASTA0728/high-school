...
%This version restricts solution support to index range [klow,khigh]
...
while k<=maxits
    ...
    %Pick out column with highest coherence in range [klow,khigh]
    ir = (1:n); ir = (ir>=klow & ir<=khigh); ir = ir';
    c = c.*ir;
    [ci,ii] = max(abs(c));
    if verbose
